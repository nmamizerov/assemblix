...
**Guiding principle:** build the streaming _mechanism_ (per-execution event log + delta
...
- A per-request **`stream: bool`** flag on `ExecuteWorkflowRequest` (does _this run_ stream).
...
- **A production, non-debug chat streaming UI** — the _mechanism_ works for any run, but the
...
This dissolves the "detect the last agent node" problem entirely: the _author_ declares
...
For the two-request subscribe model we add a **monotonic `seq`** to _every_ event on an
