...
export const JsonCodeEditor = ({
  ...
  const [value, setValue] = useState(incomingJson);
  ...
  // Подхватываем внешние изменения схемы (переключение режимов, правки в форме)
  // во время рендера — официальный React-паттерн синхронизации state с пропом.
      ...
      debounceRef.current = setTimeout(() => {
        try {
          ...
          if (
            ...
            setError(t("schemaBuilder.jsonMustBeObject"));
            return;
          }
          setError(null);
          setLastEmitted(JSON.stringify(parsed));
          onSchemaChange(parsed as OpenAPISchema);
        } catch {
          setError(t("schemaBuilder.jsonSyntaxError"));
        }
      }, 300);
    },
    [onSchemaChange, t]
